...
//         \Log::info("Testing Firebase with credentials: " . $credentialsPath);

//         if (!file_exists($credentialsPath)) {
//             return response()->json(['error' => 'Firebase credentials file not found', 'path' => $credentialsPath]);
//         }

//         $factory = (new \Kreait\Firebase\Factory)->withServiceAccount($credentialsPath);
//         $messaging = $factory->createMessaging();

//         // Just a test to see if Firebase initializes properly
//         return response()->json(['success' => true, 'message' => 'Firebase SDK initialized successfully']);
//     } catch (\Throwable $e) {
...
// For Automatic assignment of delivery boy
Route::get('/assign-delivery-boy/{secret}', [OrderController::class, 'assignDeliveryBoy'])->name('assignDelivery')->where('secret', 'test');



// For Calulating bonueses
Route::get('/bonus/process-welcome', [BonusController::class, 'processWelcomeBonus']);
Route::get('/bonus/process-weekly', [BonusController::class, 'processWeeklyBonus']);
...
Route::controller(LoginController::class)->group(function () {
    ...
    Route::post('/check/otp/status', 'checkMobileOtpStatus')->name('mobile.otpstatus');
    Route::get('/send-otp', 'sendOtp')->name('sendOtp');
    Route::post('/verify-otp', 'verifyOtp')->name('verifyNOtp');
});
...
Route::controller(HomeController::class)->group(function () {
    ...
    Route::post('/verify/phone/otp', 'verifyOtpShop')->name('verify.otp.shop');
    Route::post('/verify/email/otp', 'verifyOtpShopVerifyEmail')->name('verify.otpEmail.shop');

    Route::post('/password/reset/confirm/submit', 'confirm_password')->name('password.update.confirm');
    Route::get('/password/reset/confirm/submit/{id}', 'confirm_password_screen')->name('password.confirm.screen');
    Route::get('/otp/resend/{id}', 'resendOtp')->name('otp.resend');

    Route::get('/users/login', 'login')->name('user.login')->middleware('handle-demo-login');
    Route::get('/seller/login', 'login')->name('seller.login')->middleware('handle-demo-login', 'redirect-dashboard');
    Route::get('/seller/index', 'sellerIndex')->name('seller.index')->middleware('redirect-dashboard');
    Route::get('/deliveryboy/login', 'login')->name('deliveryboy.login')->middleware('handle-demo-login');
    Route::get('/rider', 'riderIndex')->name('deliveryboy.test')->middleware('handle-demo-login');
    ...
    Route::post('/users/registration/verify/otp', 'userOtpVerify')->name('user.verify_otp')->middleware('handle-demo-login');

    Route::post('/users/login/cart', 'cart_login')->name('cart.login.submit')->middleware('handle-demo-login');
    Route::get('/best-selling-products', 'fetchBestSellingProducts')->name('all_products');

    Route::get('/seller/index', 'sellerIndex')->name('seller.index')->middleware('handle-demo-login');
    ...
    Route::get('/hello', function () {
        ...
        return view('emails.selleremailsuccess');
    });
    ...
    Route::post('/home/section/free-shipping', 'load_free_shipping_section')->name('home.section.free_shipping');
...
Route::group(['middleware' => ['user', 'verified', 'unbanned']], function () {
    ...
    Route::controller(NotificationController::class)->group(function () {
        Route::get('/all-notifications', 'customerIndex')->name('all-notifications');
...
Route::group(['middleware' => ['customer', 'verified', 'unbanned']], function () {
    ...
    Route::controller(PurchaseHistoryController::class)->group(function () {
        ...
        Route::get('/purchase_history/details/status/{id}', 'purchase_history_details_status')->name('purchase_history.details.status');
        ...
        Route::post('current-lat-long-delivery/{id}', [PurchaseHistoryController::class, 'current_lat_long_delivery'])->name('current_lat_long_delivery');
    ...
    Route::post('/delivery-review-modal', [ReviewController::class, 'delivery_review_modal'])->name('delivery_review_modal');
...
Route::group(['middleware' => ['auth']], function () {
    ...
    Route::get('invoice/{order_id}/mini', [InvoiceController::class, 'invoice_download_mini'])->name('invoice.download.mini');
...
Route::get('/seller/verify/mobile', function () {
    return view('auth.free.verify_mobile');
})->name('seller.verify.mob');


Route::get('/bank-alfalah-callback', [CheckoutController::class, 'bankAlfalahCallback'])->name('bank_alfalah.callback');


Route::post('/payment-listener', [PaymentController::class, 'handleIPN'])->name('payment.listener');
