...
<div id="successModal" class="modal ">
    ...
    <div class="modal-content m-auto" style="max-width:440px; border-radius:16px">
        ...
        <div class="modal-style">
            ...
            <a href="{{ route('seller.dashboard') }}" class="modal-button-style btn" id="modal-ok-button">{{  translate('Dashboard') }}</a>
            @else
            <a href="{{ route('home') }}" class="modal-button-style btn" id="modal-ok-button">{{  translate('Dashboard') }}</a>
            @endif
        </div>
    </div>
  </div>
...
function handleKeydown(event, prevName) {
    ...
    $('#otp-form').on('submit', function(e) {
        ...
        $.ajax({
            url: '{{ route('verify.otpEmail.shop') }}',
            method: 'POST',
            data: $(this).serialize(),
            success: function(response) {
