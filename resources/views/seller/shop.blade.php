...
@section('panel_content')
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form id="shopUpdate" class="" action="{{ route('seller.shop.update') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="row mb-3">
                    <label class="col-md-2 col-form-label">{{ translate('Shop Logo') }} (500*500)  <span class="text-danger text-danger">*</span></label>
                    <div class="col-md-10">
                        <div class="input-group" data-toggle="aizuploader" data-type="image">
                            ...
                            <input type="hidden" name="logo" value="{{ old('logo',$shop->logo)  }}" class="selected-files" required>
                        </div>


                        <div class="file-preview box sm" id="logo">
                            @if(old('logo', $shop->logo))
                            <div class="file-preview-item">
                                <img src="{{ old('logo', $shop->logo) }}" alt="Image preview" style="width: 50px; height: 50px;">
                            </div>
                            @endif
                        </div>
                        <div id="logo-error" class="text-danger" style="display: none;">{{ translate('This field is required.') }}</div>
                ...
                <div class="row">
                    ...
                    <div class="col-md-10">
                        <input type="text" class="form-control mb-3" id="phone" placeholder="{{ translate('Phone')}}" name="phone" value="{{ $shop->phone }}" required>
                    </div>
                </div>
                <div class="row">
    ...
    <div class="card">
        ...
        <div class="card-body">
            {{-- <form class="" action="{{ route('seller.shop.update') }}" method="POST" enctype="multipart/form-data"> --}}
                ...
                <!-- Top Banner -->
                <div class="row mb-3">
                    <label class="col-md-2 col-form-label">{{ translate('Top Banner') }} (1440x375)</label>
                    <div class="col-md-10">
                        <div class="input-group" data-toggle="aizuploader" data-type="image">
                            <div class="input-group-prepend">
                                <div class="input-group-text bg-soft-secondary font-weight-medium">{{ translate('Browse')}}</div>
                            ...
                            <div class="form-control file-amount">{{ translate('Choose File') }}</div>
                            <input type="hidden" name="top_banner" value="{{ old('top_banner', $shop->top_banner)  }}" class="selected-files">
                        </div>
                        <div class="file-preview box sm">
                            @if(old('top_banner', $shop->top_banner))
                            <div class="file-preview-item">
                                <img src="{{ old('top_banner', $shop->top_banner) }}" alt="Image preview" style="width: 50px; height: 50px;">
                            </div>
                            @endif
                        </div>
                        <small class="text-muted">{{ translate('These images are visible in product details page gallery. Minimum dimensions required: 1440px width X 375px height.') }}</small>
                        @if ($errors->has('top_banner'))
                        <div class="alert alert-danger">
                                {{ $errors->first('top_banner') }}
                            </div>
                        @endif
                ...
                <!-- Slider Banners -->
                <div class="row mb-3">
                    <label class="col-md-2 col-form-label">{{ translate('Slider Banners') }}  (1440x375)</label>
                    <div class="col-md-10">
                        <div class="input-group" data-toggle="aizuploader" data-type="image" data-multiple="true">
                            ...
                            <div class="form-control file-amount">{{ translate('Choose File') }}</div>
                            <input type="hidden" name="sliders" value="{{old('sliders', $shop->sliders) }}" class="selected-files">
                        </div>
                        <div class="file-preview box sm">
                            @if(old('sliders', $shop->sliders))
                            <div class="row">
                                @foreach(explode(',', old('sliders', $shop->sliders)) as $slider)
                                <div class="col-3">
                                    <img src="{{ asset($slider) }}" alt="Slider Image" class="img-fluid">
                                </div>
                                 @endforeach
                            </div>
                            @endif
                ...
                {{-- <div class="row mb-3">
                    ...
                    <div class="col-md-10">
                        ...
                        <small class="text-muted">{{ translate('These images are visible in product details page gallery. Minimum dimensions required: 1920px width X 360px height.') }}</small>
...
@section('script')
    <script>
        ...
        $('#shopUpdate').on('submit', function(e){
          if($('#logo').is(':empty')){
            $('#logo-error').show();
            e.preventDefault();
