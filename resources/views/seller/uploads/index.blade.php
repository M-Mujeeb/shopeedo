...
<div class="card">
    <form id="sort_uploads" action="">
        <div class="card-header row gutters-5">
			...
			{{-- <div id="bulk-delete-btn"  class="dropdown mb-2 mb-md-0">
            ...
            <div id="bulk-delete-btn"  class=" mb-2 mb-md-0 ">
                <button  class="btn border " type="button" data-toggle="dropdown">
                    <a class=" confirm-alert" href="javascript:void(0)" style="color: red"  data-target="#bulk-delete-modal"> {{translate('Delete selection')}}</a>
                </button>
            </div>
...
@section('script')
    ...
    $(document).on("change", ".check-all", function() {
        ...
        toggleDeleteButton(); // Call the function to handle the button visibility
    });
    ...
    function toggleDeleteButton() {
        ...
        var totalCount = $('.check-one').length;
        var allChecked = $('.check-all').prop('checked');

        if (allChecked || selectedCount > 1) {
            $('#bulk-delete-btn').removeClass('d-none').addClass('d-block');
        } else {
            $('#bulk-delete-btn').removeClass('d-block').addClass('d-none');
        }
    }
});
