...
<div class="card">
    <div class="card-header d-block d-lg-flex">
        ...
        <div class="">
            ...
            <a href="{{ route('delivery-boy.export-delivery-boys') }}"  class="text-align:end btn btn-primary" style="float: right; margin-top:5px">Export</a>
        ...
        <table class="table aiz-table mb-0">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        {{ single_price($delivery_boy->total_bonus) }}
                    </td>
                    <td>
                        @if($delivery_boy->status == 1)
                            <span class="badge bg-success px-4 text-white">Online</span>
                        @else
                            <span class="badge bg-danger px-4 text-white">Offline</span>
                        @endif
                    </td>

                    <td>{{date('d-m-Y', strtotime($delivery_boy->user->created_at))}}</td>
                    <td>
                        <div class="dropdown">
                            ...
                            <div class="dropdown-menu dropdown-menu-right dropdown-menu-xs">
                                ...
                                @endcan
                                @can('bonus_to_delivery_boy')
                                    <a href="#" onclick="show_delivery_bonus_modal('{{$delivery_boy->user->id}}');" class="dropdown-item">
                                        {{translate('Go to Bonus')}}
                                    </a>
                                @endcan
                            </div>
                        </div>
...
<div class="modal fade" id="bonus_modal">
    <div class="modal-dialog">
        <div class="modal-content" id="bonus-modal-content">
...
@section('script')
    <script type="text/javascript">
        ...
        function show_delivery_bonus_modal(id){
            $.post('{{ route('delivery-boy.delivery-bonus') }}',{
                _token  :'{{ @csrf_token() }}',
                id      :id
            }, function(data){
                $('#bonus_modal #bonus-modal-content').html(data);
                $('#bonus_modal').modal('show', {backdrop: 'static'});
            });
        }
