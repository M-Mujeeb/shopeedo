                    ...
                    @if(json_decode($order->shipping_address))
                        <address>
                            ...
                            {{ json_decode($order->shipping_address)->address }}, {{ json_decode($order->shipping_address)->city }},
                             ...
                             @endif
                              {{ isset(json_decode($order->shipping_address)->postal_code) }} && {{ isset(json_decode($order->shipping_address)->postal_code) }} <br>
            ...
            <div class="clearfix float-right">
                <table class="table">
                    <tbody>
                        ...
                        <tr>
                            <td>
                                <strong class="text-muted">{{ translate('Platform Fee') }} :</strong>
                            </td>
                            <td>
                                {{ single_price($platform_fee->value) }}
                            </td>
                        </tr>
                        <tr>
                            <td>
...
@section('modal')
    ...
    <div id="confirm-payment-status" class="modal fade">
     ...
     <!-- reason Status Modal -->
     <div id="cancel-reason" class="modal fade">
        <div class="modal-dialog modal-md modal-dialog-centered" style="max-width: 540px;">
            <div class="modal-content p-2rem">
                <div class="modal-body text-center">
                    <form action="">
                        <label for="reason">Reason: </label>
                        <input class="p-2" type="text" name="cancel-reason" placeholder="reason of cancellation" value="">
                    </form>
                    {{-- <p class="mt-3 mb-3 fs-16 fw-700">{{translate('Are you sure you want to change the payment status?')}}</p> --}}
                    {{-- <button type="button" class="btn btn-light rounded-2 mt-2 fs-13 fw-700 w-150px" data-dismiss="modal">{{ translate('Cancel') }}</button> --}}
                    <button type="button" onclick="reasonForCancellation()" class="btn text-white rounded-2 mt-2 fs-13 fw-700 w-150px" style="background-color:#7d9a40">{{translate('submit reason')}}</button>
                </div>
            </div>
        </div>
    </div>
...
@section('script')
    <script type="text/javascript">
        ...
        //         order_id: order_id,
        //         status: status,
        //         // reason:  $("input[name='cancel-reason']").val(),
    ...
    if (status == 'cancelled') {
        ...
        $.post('{{ route('orders.update_delivery_status') }}', {
            ...
            // Check if the status is "confirmed" and update delivery boy dropdown
            if (status === 'confirmed') {
                $.get('{{ route('orders.get-delivery-boys', ['order_id' => $order->id]) }}', function(response) {
                    // Update the dropdown with new delivery boys
                    let dropdown = $('#assign_deliver_boy');
                    dropdown.empty(); // Clear existing options
                    dropdown.append(`<option value="">{{ translate('Select Delivery Boy') }}</option>`);

                    response.delivery_boys.forEach(function(delivery_boy) {
                        dropdown.append(`
                            <option value="${delivery_boy.id}"
                                ${response.assigned_delivery_boy == delivery_boy.id ? 'selected' : ''}>
                                ${delivery_boy.name}
                            </option>
                        `);
                    });

                    // Reinitialize the dropdown for AIZ Selectpicker
