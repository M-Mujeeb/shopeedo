<div class="modal-body">
  ...
  <h6 class="mb-4">{{translate('National Id Info')}}</h6>
  <p>{{translate('Cnic No')}} : {{ $user->cnic_no }}</p>
  <p>{{translate('Date of Issue')}} : {{ $user->date_of_issue }}</p>
  <p>{{translate('Date of Expiry')}} : {{ $user->date_of_expiry }}</p>

  <p >Front Side Picture:</p>
   <img src="{{ uploaded_asset($user->front_side_picture) }}" width="300" height="300">
   <p class="mt-3">Back Side Picture:</p>
   <img src="{{ uploaded_asset($user->back_side_picture) }}" width="300" height="300"/>
