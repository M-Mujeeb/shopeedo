...
<body>
	<div>
		...
		@php
            ...
            $data = date('d-m-Y', $order->date);
		@endphp

		<div style="background: #eceff4;padding: 1rem;">
					@php
					$image_path = uploaded_asset($logo);
					$image_path = str_replace('https://localhost', 'http://localhost', $image_path);
					@endphp
				...
				<tr>
					<td>
						 ...
						 @if($logo != null)
							...
							<img src="{{ $image_path }}" height="30" style="display:inline-block;">
						@else
		...
		<div style="padding: 1rem;padding-bottom: 0">
            <table>
                ...
                <tr><td class="gry-color small">{{ $shipping_address->address }}, {{ $shipping_address->city }},  @if(isset(json_decode($order->shipping_address)->state)) {{ json_decode($order->shipping_address)->state }} - @endif</td></tr>
			...
			<table class="padding text-left small border-bottom">
				<thead>
	                <tr class="gry-color" style="background: #eceff4;">
	                    ...
	                    {{-- <th width="10%" class="text-left">{{ translate('Platform Fees') }}</th> --}}
				...
				<tbody class="strong">
	                @foreach ($order->orderDetails as $key => $orderDetail)
                        ...
                        @php
                        $order->payment_type = $order->payment_type == 'cash_on_delivery' ? 'Cash on Delivery' : 'Bank';

                        $orderQrDetails =
                            'Invoice Number: ' . $order->code . "\n" .
                            'Order Date: ' . $data . "\n" .
                            'Personal Name: ' . $shipping_address->name . "\n" .
                    ...
                    @endphp
							...
							<tr class="">
			                    ...
			                    {{-- <td class="text-right currency">{{ single_price($platform_fees) }}</td> --}}
		        ...
		        <tbody>
			        <tr>
			            <td class="text-left">
                            ...
                            {!! str_replace($removedXML,"", QrCode::size(100)->generate($orderQrDetails)) !!}
			            </td>
			            <td>
							@php
					        ...
					        <table class="text-right sm-padding small strong">
						        <tbody>
							        ...
							        <tr>
							            ...
							            <td class="currency">{{ single_price($new_shipping_cost? $order->combinedOrder->shipping_cost : $order->orderDetails->sum('shipping_cost')) }}</td>
							        </tr>
                                    ...
                                    <tr class="border-bottom">
							            <th class="gry-color text-left">{{ translate('Platform Fees') }}</th>
							            <td class="currency">{{ single_price($platform_fees) }}</td>
							        </tr>
							        <tr>
