            ...
            <div class="col-md-2 ml-auto">
                <select class="form-control form-control-sm aiz-selectpicker mb-2 mb-md-0" id="user_id" name="user_id" onchange="sort_products()">
                    ...
                    @foreach (App\Models\User::where('user_type', '=', 'seller')->get() as $key => $seller)
                        @if ($seller->shop && $seller->shop->name && $seller->name)
                    ...
                    <tr>
                        ...
                        <td>
                            <label class="aiz-switch aiz-switch-success mb-0">
                                <input onchange="update_todays_deal(this)" value="{{ $product->id }}" type="checkbox" {{ $product->published == 1 ? 'checked' : '' }} >
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="unpublishModalLabel">Reason for Unpublishing Product</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <!-- Form to submit the reason -->
                <form id="unpublishedReason" action="{{ route('products.published') }}" method="POST">
                    @csrf
                    <div class="form-group">
                        <input type="hidden" name="productId" id="product_id" value>
                        <label for="reason">Reason for Unpublished Product</label>
                        <textarea name="reason" id="reason" class="form-control" required></textarea>

                    </div>
                    <button type="submit" class="btn btn-primary" onclick="reasonToUnplublished(event)">Submit</button>
                </form>
            </div>
        </div>
    </div>
...
@section('script')
    <script type="text/javascript">
        ...
        if (!el.checked) {
            ...
            // Directly update the product if checked (published)
            var status = 1;
            $.post('{{ route('products.published') }}', {
                ...
                if (data == 1) {
                    AIZ.plugins.notify('success', '{{ translate('Product published successfully') }}');
                } else {
                    AIZ.plugins.notify('danger', '{{ translate('Something went wrong') }}');
                }
            });
    ...
    function reasonToUnplublished(e) {
       e.preventDefault();

    // Get the product ID and reason from the form fields
    var productId = $('#product_id').val(); // Fetch product ID from hidden input
    var reason = $('#reason').val(); // Fetch reason from textarea

    var status = 0;

    // Send AJAX POST request to update the product's status and reason for unpublishing
    $.post('{{ route('products.published') }}', {
        _token: '{{ csrf_token() }}',
        id: productId,  // Send the product ID
        status: status,  // Send the status as unpublished
        reason: reason  // Send the reason for unpublishing
    }, function(data) {
        if (data == 1) {
            // Notify success
            AIZ.plugins.notify('success', '{{ translate('Product unpublished successfully') }}');

            // Optionally, close the modal after successful submission
            $('#unpublishModal').modal('hide');
            $('input[value="' + productId + '"]').prop('checked', true);
        } else {
            // Notify failure
            AIZ.plugins.notify('danger', '{{ translate('Something went wrong') }}');
        }
