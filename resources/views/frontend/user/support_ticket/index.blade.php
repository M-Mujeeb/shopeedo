...
@section('panel_content')
    ...
    <div class="card rounded-0 shadow-none border">
        <div class="card-header border-bottom-0">
            <h5 class="mb-0 fs-20 fw-700 p-2 text-dark text-center text-md-left">{{ translate('Tickets')}}</h5>
        </div>
          <div class="card-body py-0">
              <table class="table aiz-table mb-4">
                  <thead class="text-gray fs-12">
                      <tr>
                          <th data-breakpoints="lg" class="pl-2">{{ translate('Ticket ID') }}</th>
                          <th data-breakpoints="lg">{{ translate('Sending Date') }}</th>
                          <th>{{ translate('Subject')}}</th>
                          <th>{{ translate('Status')}}</th>
                          <th data-breakpoints="lg" class="text-right pr-2">{{ translate('Options')}}</th>
                      </tr>
                  </thead>
                  <tbody class="fs-14">
                      @foreach ($tickets as $key => $ticket)
                          <tr>
                              <td class="pl-2 fw-700">#{{ $ticket->code }}</td>
