...
@if (!empty($seller_products))
    ...
    @foreach ($seller_products as $key => $seller_product)
        <div class="card @if($loop->last) mb-0 @else mb-3 @endif border-left-0 border-top-0 border-right-0 @if($loop->last) border-bottom-0 @else border-bottom @endif rounded-0 shadow-none">
            ...
            <div class="card-body p-0">
                {{-- 'all_sellers' =>$owner_ids
                , 'delivery_show' => $delivery_show ,--}}
                @include('frontend.partials.cart.delivery_info_details', ['products' => $seller_product, 'product_variation' => $seller_product_variation, 'owner_id' => $key, 'delivery_show' => $delivery_show ,  ])
            </div>
        </div>
    @php
    $delivery_show = false;
