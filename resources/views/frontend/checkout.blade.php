            ...
            <div class="row cols-xs-space cols-sm-space cols-md-space">
                ...
                <div class="col-lg-4 mt-4 mt-lg-0" id="cart_summary">
                    @include('frontend.partials.cart.cart_summary', [
                        'proceed' => 0,  // 0 for cart screen and 1 for final checkout screen
                        'carts' => $carts,
...
@section('script')
    <script type="text/javascript">
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function updatePaymentUI(selectedOption) {
                if (selectedOption === 'card_payment') {
                    cardPaymentForm.classList.remove('d-none');
                    submitOrderBtn.textContent = '{{ translate('Pay to Complete Order') }}';
                } else {
                    cardPaymentForm.classList.add('d-none');
                    submitOrderBtn.textContent = '{{ translate('Complete Order') }}';
                }
            }
            ...
            // Initialize UI based on the initially selected payment option
            const initialSelectedOption = document.querySelector('input[name="payment_option"]:checked');
            if (initialSelectedOption) {
                updatePaymentUI(initialSelectedOption.value);
            }
        });
        ...
        function submitOrder(el) {
            ...
            if ($('#agree_checkbox').is(":checked")) {
                if (minimum_order_amount_check && $('#sub_total').val() < minimum_order_amount) {
                    AIZ.plugins.notify('danger',
                        '{{ translate('Your order amount is less than the minimum order amount') }}');
                    ...
                    if (selectedPaymentOption === 'card_payment') {
                        // Validate card details
                        var cardNumber = $('#card_number').val();
                        var expiryDate = $('#expiry_date').val();
                        var cvv = $('#cvv').val();
                        if (!cardNumber || !expiryDate || !cvv) {
                            AIZ.plugins.notify('danger', '{{ translate('Please fill in all card details') }}');
                            $(el).prop('disabled', false);
                            return;
                        }
                        // Add card payment processing logic here
                    } else if (offline_payment_active == '1' && $('.offline_payment_option').is(":checked") && $('#trx_id')
                        .val() == '') {
                    ...
                    if (allIsOk) {
                        if (selectedPaymentOption === 'card_payment') {
                            ...
                            // This could involve making an AJAX call to your payment gateway
                            // and then submitting the form on success
                            processCardPayment(function(success) {
                                if (success) {
                                    $('#checkout-form').submit();
                                } else {
                                    AIZ.plugins.notify('danger',
                                        '{{ translate('Card payment failed. Please try again.') }}');
                                    $(el).prop('disabled', false);
                                }
                            });
                        } else {
                            // For non-card payments, submit the form directly
                            $('#checkout-form').submit();
                        }
                    } else {
                        $(el).prop('disabled', false);
        ...
        function processCardPayment(callback) {
            // Simulating an asynchronous payment process
            setTimeout(function() {
                // For demonstration, we're assuming the payment is always successful
                // In a real implementation, this would involve communicating with your payment gateway
        ...
        $(document).on('click', '[data-code]', function () {
            var couponCode = $(this).attr('data-code'); // Get the value of data-code

            $.ajax({
                url: "{{ route('checkout.apply_coupon_code') }}", // Laravel route
                type: "POST",
                data: {
                    code: couponCode,
                ...
                success: function (response) {
                    ...
                    // alert("Coupon Applied: " + couponCode + " - " + response.message);
                },
                error: function (xhr) {
        ...
        function stepCompletionPaymentInfo() {
            ...
            if (selectedPaymentOption) {
                if (selectedPaymentOption === 'card_payment') {
                    ...
                    payment = true;
                } else if ($('input[name="payment_option"]:checked').hasClass('offline_payment_option')) {
