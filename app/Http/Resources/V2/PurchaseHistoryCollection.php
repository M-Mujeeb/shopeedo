    ...
    public function toArray($request)
    {
        return [
            'data' => $this->collection->map(function ($data) {
                ...
                if ($data->assign_delivery_boy != null && $data->delivery_status == 'delivered') {
                    $delivery_boy = DeliveryBoy::where('user_id', $data->assign_delivery_boy)->first();
                    if ($delivery_boy) {
                        $isReviewed = Rating::where('delivery_boy_id', $delivery_boy->id)
                ...
                $customer = Address::where('user_id', $data->user_id)->where('set_default', 1)->first();
                ...
                return [
                   ...
                   'seller_lat' => (float)($seller->delivery_pickup_latitude ?? 0.0),
                    'seller_lon' => (float)($seller->delivery_pickup_longitude ?? 0.0),
                    'customer_lat' => (float)($customer->latitude ?? 0.0),
                    'customer_lon' => (float)($customer->longitude ?? 0.0),
                    ...
                    'name' => $delivery_boy ? ($delivery_boy->user->name ?? '') : '',
                    'phone' => $delivery_boy ? ($delivery_boy->user->phone ?? '') : '',
                    'image' => $delivery_boy ? ($delivery_boy->user->avatar_original ? uploaded_asset($delivery_boy->user->avatar_original) : '') : '',
                    'ratings' => $delivery_boy ? $delivery_boy->rating : 0,
