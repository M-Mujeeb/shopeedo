...
// shop and customer distance calculation
if (!function_exists('customer_shop_distance')) {
    function customer_shop_distance($lat1, $lon1, $lat2, $lon2)
    {
...
function getMultipleRoutes($lat1, $lon1, $lat2, $lon2)
{
    ...
    $url = "https://maps.googleapis.com/maps/api/directions/json?origin={$origin}&destination={$destination}&alternatives=true&departure_time=now&key={$apiKey}";
...
if (!function_exists('get_free_shipping_products')) {
    function get_free_shipping_products($limit)
    {
        $product_query = Product::query();
        // if ($user_id) {
        //     $product_query = $product_query->where('user_id', $user_id);
        // }
        return filter_products($product_query->where('shipping_type', 'free'))->limit($limit)->get();
    }
}

if (!function_exists('get_platform_fees')) {
    function get_platform_fees()
    {
        $platform_fees = BusinessSetting::where('type', 'platform_fee')->first();
        $platform_fees = $platform_fees->value;
...
if (!function_exists('flat_rate_shipping_cost')) {
    function get_shippment_fees()
    {
        $shipment_fees = BusinessSetting::where('type', 'flat_rate_shipping_cost')->first();
        $shipment_fees = $shipment_fees->value;
...
if (!function_exists('get_seller_products')) {
    ...
    {
        $product_query = Product::query();
        // dd($product_query->where('user_id', $user_id)->isApprovedPublished()->where('current_stock', '>', 0)->orderBy('created_at', 'desc')->limit(15)->get());
        return $product_query->where('user_id', $user_id)->isApprovedPublished()->where('current_stock', '>', 0)->orderBy('created_at', 'desc')->limit(15)->get();
    }
}
...
if (!function_exists('get_paginated_sellers')) {
    function get_paginated_sellers($limit = 10)
    {
        return Cache::remember('best_sellers_page_' . request('page', 1), 86400, function () use ($limit) {
...
function sendLowStockProductEmail($product_name, $seller_name, $email)
{
    $array['view'] = 'emails.verification';
    $array['from'] = env('MAIL_FROM_ADDRESS');
    $array['subject'] = translate('Important: Low Stock Alert for Your Products on Shopeedo');

    $array['content'] = '
            Dear ' . $seller_name . ',<br><br>

            We hope this message finds you well. As part of our commitment to helping you succeed on Shopeedo, we want to inform you that some of your products are currently running low on stock. Maintaining adequate stock levels is crucial to ensuring that you do not miss out on potential sales opportunities.

            <strong>Low Stock Product</strong><br>'
        . $product_name . '
           <strong>Why it important to restock</strong>
            1. <strong>Avoid Missed Sales: </strong> Low stock can lead to missed sales opportunities, especially if customers find your products out of stock when they are ready to purchase.<br>
            2. <strong>Improve Customer Satisfaction: </strong> Having products readily available ensures a positive shopping experience for customers, which can lead to repeat business and positive reviews.<br>
            3. <strong>Maintain Your Rankings: </strong> Consistently available products tend to perform better in search rankings, which can increase visibility and sales on Shopeedo.<br><br>

            <strong>Action Required:</strong> Please review your inventory levels and restock the above-listed products as soon as possible. If you encounter any issues with restocking or need assistance, our support team is here to help you. You can reach us at info@shopeedo.com.
            <strong>How to Update Your Inventory:</strong>
            1.	Log in to your Shopeedo seller account.
            2.	Navigate to the Inventory section.
            3.	Update the stock quantities for the listed products.
            4.	Save your changes.
            <br><br>
            <strong>Need Help?</strong> If you have any questions or need assistance with updating your inventory, please do not hesitate to contact our support team. We are here to ensure your selling experience on Shopeedo is smooth and successful.<br><br>

            Thank you for your prompt attention to this matter. We appreciate your continued partnership and look forward to your products being fully stocked and available for our customers.
...
function getAutoAssignedDeliveryBoy($order)
{
    ...
    $deliveryBoys = DeliveryBoy::whereIn('user_id', $delivery_boys->pluck('id'))
        ...
        \Log::info("getAutoAssignedDeliveryBoy : {$deliveryBoys}");

    $availableRiders = $deliveryBoys->filter(function ($rider) {
        return !$rider->isAssignedToOrder();
    });
    ...
    $sortedRiders = $availableRiders->sort(function ($a, $b) {
    ...
    $selectedRider = User::find($sortedRiders->first()->user_id);

    return $selectedRider->id ?? null;
}

if (!function_exists('format_float')) {
    function format_float($value, $precision) {
        return (float) sprintf("%.{$precision}f", (float) $value);
    }
}
...
function codOf(){
    if(auth()->check()){
        ...
        ->where('cancel_by', 'customer')
        ->latest()  // Orders by created_at descending
        ->take(3)   // Limits to 3 records
        ->get()
        ->count();  // Count the retrieved collection
    return $user_orders;
    }
}
