...
class CheckoutController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($carts && count($carts) > 0) {
            ...
            $total = $subtotal + $tax + $shipping + $platform_fees;
            ...
            $coupons = [
                ['name' => 'Hafsaa', 'amount' => 250, 'currency' => 'PKR'],
                ['name' => 'Discount50', 'amount' => 500, 'currency' => 'PKR'],
                ['name' => 'SummerSale', 'amount' => 100, 'currency' => 'PKR'],
            ];

            return view('frontend.checkout', compact('carts', 'address_id', 'total', 'carrier_list', 'shipping_info', 'platform_fees', 'cancel_orders', 'coupons'));
        }
    ...
    public function checkout(Request $request)
    {
        ...
        // dd($combined_order_id);

        if ($combined_order_id == 0) {
            flash(translate('Your order could not be processed. Please try again.'))->error();
            return redirect()->route('checkout');
        }
        ...
        if ($request->payment_option == 'bank_alfalah_card') {
            // Bank Alfalah card payment integration
            $merchant_id = config('payment.bank_alfalah.merchant_id');
            $store_id = config('payment.bank_alfalah.store_id');
            $merchant_hash = config('payment.bank_alfalah.merchant_hash');
            $merchant_username = config('payment.bank_alfalah.merchant_username');
            $merchant_password = config('payment.bank_alfalah.merchant_password');

            $post_data = [
                'HS_RequestHash' => $this->generateRequestHash($combined_order),
                'HS_IsRedirectionRequest' => '1',
                'HS_ChannelId' => '1001',
                'HS_ReturnURL' => route('bank_alfalah.callback'),
                ...
                'HS_TransactionReferenceNumber' => $combined_order->code,
            ];

            // Redirect to Bank Alfalah payment gateway
            return view('payment.bank_alfalah_redirect', compact('post_data'));
        } else {


             session()->put('order_id',$combined_order->orders[0]->id);
    ...
    private function generateRequestHash($combined_order)
    {
        // Implement the hash generation logic as per Bank Alfalah's documentation
        // This is a placeholder implementation
        $merchant_id = config('payment.bank_alfalah.merchant_id');
        $store_id = config('payment.bank_alfalah.store_id');
        $merchant_hash = config('payment.bank_alfalah.merchant_hash');

        $hash_string = $merchant_id . $store_id . $combined_order->code . $combined_order->grand_total . $merchant_hash;
        return hash('sha256', $hash_string);
    }

    public function bankAlfalahCallback(Request $request)
    {

        $auth_token = $request->input('auth_token');

        $response = Http::post('https://payments.bankalfalah.com/SSO/SSO/SSO', [
            'AuthToken' => $auth_token,
            'RequestHash' => $this->generateRequestHash($auth_token),
            'ChannelId' => '1001',
            'Currency' => 'PKR',
            'ReturnURL' => route('order_confirmed'),
            'MerchantId' => config('payment.bank_alfalah.merchant_id'),
            'StoreId' => config('payment.bank_alfalah.store_id'),
            'MerchantHash' => config('payment.bank_alfalah.merchant_hash'),
            'MerchantUsername' => config('payment.bank_alfalah.merchant_username'),
            'MerchantPassword' => config('payment.bank_alfalah.merchant_password'),
            'TransactionTypeId' => '3',
        ]);

        if ($response->successful() && $response['TransactionStatus'] == 'PAID') {
            $combined_order = CombinedOrder::where('code', $response['TransactionReferenceNumber'])->first();
            if ($combined_order) {
                foreach ($combined_order->orders as $order) {
            ...
            flash(translate('Your payment has been completed successfully.'))->success();
            return redirect()->route('order_confirmed');
        } else {
    ...
    public function apply_coupon_code(Request $request)
    {
        ...
        $returnHTML = view('frontend.partials.cart.cart_summary', compact('coupon', 'carts', 'proceed', 'platform_fees'))->render();
    ...
    public function remove_coupon_code(Request $request)
    {
        ...
        return view('frontend.partials.cart.cart_summary', compact('coupon', 'carts', 'proceed', 'platform_fees'));
    }
    ...
    public function updateDeliveryAddress(Request $request)
    {
        ...
        $update_address_status = Address::findOrFail($request->address_id);
        $update_address_status->set_default = 1;
        $update_address_status->save();
        // dd($update_address_status);
        ...
        return array(
            ...
            'cart_summary' => view('frontend.partials.cart.cart_summary', compact('carts', 'proceed', 'platform_fees'))->render()
        );
    }
