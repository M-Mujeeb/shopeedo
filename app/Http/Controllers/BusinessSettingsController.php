...
class BusinessSettingsController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        if ($request->home_slider_images != null) {
            foreach ($request->home_slider_images as $key => $slider_image) {
                if ($slider_image != null) {
                    try {
                        $image = Upload::where('id', $slider_image)->first();

                        if (!$image) {
                            $errors["home_slider_images.{$key}"] = "Image not found.";
                            continue;
                        }

                        $filePath = Storage::path($image->file_name);
                        ...
                        if ($validator->fails()) {
                            $errors["home_slider_images.{$key}"] = $validator->errors()->first('image');
                        }
                    } catch (\Exception $e) {
                        $errors["home_slider_images.{$key}"] = "Error processing image: " . $e->getMessage();
                    }
                }
                // If there are any errors, redirect back with all errors
                // dd($errors);
                if (!empty($errors)) {
...
{
    // Store Welcome Bonuses
    $welcomeBonuses = [];
    if ($request->welcome_deliveries && $request->welcome_prices) {
        foreach ($request->welcome_deliveries as $index => $delivery) {
            if (!empty($delivery) && isset($request->welcome_prices[$index])) {
                $welcomeBonuses[] = [
                    'deliveries' => intval($delivery),
                    'price' => intval($request->welcome_prices[$index])
                ];
            }
        }
    }

    // Store Weekly Bonuses
    $weeklyBonuses = [];
    if ($request->weekly_deliveries && $request->weekly_prices) {
        foreach ($request->weekly_deliveries as $index => $delivery) {
            if (!empty($delivery) && isset($request->weekly_prices[$index])) {
                $weeklyBonuses[] = [
                    'deliveries' => intval($delivery),
                    'price' => intval($request->weekly_prices[$index])
                ];
            }
        }
    }

    // Save Welcome Bonuses to Business Settings
    BusinessSetting::updateOrCreate(
        ['type' => 'welcome_bonuses'],
        ['value' => json_encode($welcomeBonuses)]
    );

    // Save Weekly Bonuses to Business Settings
    BusinessSetting::updateOrCreate(
        ['type' => 'weekly_bonuses'],
        ['value' => json_encode($weeklyBonuses)]
    );

    Artisan::call('cache:clear');
    flash(translate("Bonuses updated successfully"))->success();
    return redirect()->back();
}
    ...
    public function shipping_configuration_update(Request $request)
    {
        ...
        if($request->has('per_km')) {
        $business_settings = BusinessSetting::where('type', 'per_km')->first();
        $business_settings->value = $request->per_km;
        $business_settings->save();
        }
