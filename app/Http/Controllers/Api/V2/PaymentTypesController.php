...
class PaymentTypesController
{
    ...
    public function getList(Request $request)
    {
        ...
        $total = 0;
        $subtotal = 0;        $tax = 0;
        $platform_fees = get_setting('platform_fee');
        ...
        $total = $subtotal + $shipping + $platform_fees;


        // Cancel & minmum thred cod dekhan
        $max_cod_amount = get_setting('max_cod_amount');
        $show_cod = true;

        if ($max_cod_amount && $total > $max_cod_amount) {
            $show_cod = false;
        }
        ...
        if ($list == "online" || $list == "both") {
            ...
            if (count($all_online_payment_methods) > 0) {
                $available_online_payment_methods = [
                    "paypal", "stripe", "instamojo", "razorpay", "paystack", "iyzico", "bkash", "nagad", "sslcommerz", "aamarpay", "flutterwave", "payfast", "paytm", "khalti", "myfatoorah", "phonepe", "card_payment"
                ];
                ...
                foreach ($online_payment_methods as $online_payment_method){
                    if ($online_payment_method->active == 1) {
                        ...
                        $payment_type['image'] = static_asset('assets/img/cards/'.$online_payment_method->name.'.svg');
                        $payment_type['name'] = ucfirst($online_payment_method->name);
                        $payment_type['title'] = ($online_payment_method->name == 'card_payment') ? translate("Card Payment") : translate("Checkout with ".$online_payment_method->name);
        ...
        if ($mode != 'wallet' && $mode != 'seller_package' && $list != "offline") {
            ...
            if (get_setting('cash_payment') == 1  && !$haveDigitalProduct && !$cash_on_delivery && $show_cod) {
                $payment_type = array();
                $payment_type['payment_type'] = 'cash_payment';
                $payment_type['payment_type_key'] = 'cash_on_delivery';
                $payment_type['image'] = static_asset('assets/img/cards/cod.svg');
