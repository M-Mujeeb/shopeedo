...
class ProductController extends Controller
{
    ...
    {
        $products = Product::where('shipping_type', 'free')
            ->whereHas('category', function ($query) {
                $query->where('is_quick', 1);
            })
            ->physical();
    ...
    public function liveSearch(Request $request)
{
    ...
    $products = Product::query()
        ...
        ->where(function($q) use ($query) {
            $q->where('name', 'like', $query . '%')
              ...
              ->orWhereHas('product_translations', function ($subQuery) use ($query) {
    ...
    return response()->json([
        'results' => $products->map(function($product) {
            return [
                ...
                'name' => $product->name,
                'slug' => $product->slug,
                'thumbnail' =>  uploaded_asset($product->thumbnail_img) ? uploaded_asset($product->thumbnail_img) : "",
    ...
    public function subCategoryProducts($id){
        $products = Product::where('category_id', $id)->physical();
        return new ProductMiniCollection(filter_products($products)->latest()->paginate(10));
    }
}
