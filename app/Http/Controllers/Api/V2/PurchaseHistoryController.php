...
class PurchaseHistoryController extends Controller
{
    ...
    {
        $order_detail = Order::with('combinedOrder')->where('id', $id)->where('user_id', auth()->user()->id)->get();
    ...
    public function rateDeliveryBoyStore(Request $request)
    {
        ...
        // Check if the user has already rated this delivery boy for this order
        ...
        if ($existingRating) {
            return response()->json([
                'status'  => false,
                'message' => 'You have already rated this delivery boy for this order.',
                "reviewable" => !$existingRating
        ...
        $ratingValue = floatval($request->rating);
        ...
        if ($request->is_cancel) {
            ...
            return response()->json([
                'status'  => true,
                'message' => 'Rating submission canceled.',
            ], 200);
        }

        // Create a new rating entry in the ratings table
        $rating = Rating::create([
            'delivery_boy_id' => $request->delivery_boy_id,
            'user_id'         => $user->id,
            'rating'          => $ratingValue,
        ...
        // Calculate the new average rating for the delivery boy
        $oldTotalRating = $deliveryBoy->rating * $deliveryBoy->rating_count;
        $newTotalRating = $oldTotalRating + $ratingValue;
        $newRatingCount = $deliveryBoy->rating_count + 1;
        $newAverageRating = $newTotalRating / $newRatingCount;

        // Update the delivery boy's rating and rating count
        $deliveryBoy->rating = round($newAverageRating, 2); // Ensure two decimal places
        $deliveryBoy->rating_count = $newRatingCount;
        $deliveryBoy->save();
