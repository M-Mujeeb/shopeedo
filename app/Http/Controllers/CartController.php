...
class CartController extends Controller
{
    public function index(Request $request)
    {
        ...
        if (auth()->user() != null) {
            ...
            $owner_id = Cart::where('user_id', $user_id)->active()->select('owner_id')->groupBy('owner_id')->pluck('owner_id')->toArray();
        } else {
        ...
        // $coupons = Coupon::where('start_date', '<=', strtotime(date('d-m-Y')))->where('end_date', '>=', strtotime(date('d-m-Y')))->where('user_id', $owner_id[0])->get();
        // // dd($coupons);
        ...
        return view('frontend.view_cart', compact('carts', 'platform_fees'));
    }
    ...
    public function removeFromCart(Request $request)
    {
        ...
        return array(
            'cart_count' => count($carts),
            'cart_view' => view('frontend.partials.cart.cart_details', compact('carts', 'platform_fees'))->render(),
            'nav_cart_view' => view('frontend.partials.cart.cart')->render(),
        );
    }
    ...
    public function updateCartStatus(Request $request)
    {
        ...
        return view('frontend.partials.cart.cart_details', compact('carts', 'platform_fees'))->render();
    }
}
