...
class ProductService
{
    public function store(array $data)
    {
        ...
        // dd($collection['category_ids']);
        // dd($data['category_ids']);
        $category_id = "";
        foreach ($collection['category_ids'] as $category) {
            $category = Category::where('id', $category)->where('parent_id', 0)->first();
            if ($category != null) {
        ...
        if (isset($collection['tags'][0]) && $collection['tags'][0] != null) {
            $tagsString = $collection['tags'][0];  // Example: "html,csss"

            // Split the string by commas to get individual tags
            $tagsArray = explode(',', $tagsString);

            // Add double quotes around each tag and convert it into a valid JSON string
            $validJsonString = json_encode($tagsArray);
            $decoded_tags = json_decode($validJsonString);
